...
def material_group(material_name):
    ...
    if "отсев" in text:
        return "Отсев"
    if "керамзит" in text or "кермазит" in text:
        return "Керамзит"
...
def normalize_material_search(value):
    """Нормализует пользовательский поиск и частые опечатки."""
...
def material_family(material_name):
    ...
    known_families = (
        ...
        (("щпс",), "ЩПС"),
        (("отсев",), "Отсев"),
        (("асфальтовая крошка", "асфальтная крошка"), "Асфальтовая крошка"),
        (("бетонный бой", "бой бетона"), "Бетонный бой"),
        (("кирпичный бой", "бой кирпича"), "Кирпичный бой"),
        (("чернозем",), "Чернозём"),
        (("торф",), "Торф"),
        (("глина",), "Глина"),
        (("грунт",), "Грунт"),
    ...
    clean_name = re.split(r"[\(\[,;/]", str(material_name or ""), maxsplit=1)[0].strip(" -—")
...
def build_material_suggestions(df):
    """Создаёт поисковые подсказки: сначала «любой», затем точные позиции."""
    products = sorted(
        {str(value).strip() for value in df["Вид товара"].dropna() if str(value).strip()},
        key=lambda value: normalize_material_search(value),
    )
    products_by_family = {}
    for product in products:
        family = material_family(product)
        family_key = normalize_material_search(family)
        products_by_family.setdefault((family_key, family), []).append(product)

    suggestions = [{
        "value": "all",
        "label": "Любой материал / ближайший карьер",
        "family": "all",
        "kind": "any",
        "search": "любой материал ближайший карьер",
    }]
    for (family_key, family), family_products in sorted(
        products_by_family.items(),
        key=lambda item: normalize_material_search(item[0][1]),
    ):
        suggestions.append({
...
def filter_material_selection(df, selection):
    ...
    if selection.startswith("product:"):
        exact_product = selection.split(":", 1)[1]
        return df[df["Вид товара"].astype(str) == exact_product]
    if selection.startswith("family:"):
        family_key = selection.split(":", 1)[1]
        family_mask = df["Вид товара"].apply(
            lambda value: normalize_material_search(material_family(value)) == family_key
        )
        return df[family_mask]

    # Обратная совместимость со старым select и сохранёнными ссылками.
...
def home():
    ...
    material_suggestions = build_material_suggestions(df)
    material_suggestions_json = json.dumps(
    ...
    selected_material_value = request.form.get("sand_type", "") if request.method == "POST" else ""
    selected_material_label = ""
    for suggestion in material_suggestions:
        if suggestion["value"] == selected_material_value:
            selected_material_label = suggestion["label"]
            break
    ...
    <head>
        ...
        <style>
            ...
            .material-search input {margin-bottom:0;}
            .material-hint {display:block; color:#667085; font-size:13px; margin:7px 0 18px;}
            .material-dropdown {
                display:none; position:absolute; z-index:1000; top:58px; left:0; right:0;
                max-height:360px; overflow-y:auto; background:#fff; border:1px solid #ccd5e1;
                border-radius:12px; box-shadow:0 14px 32px rgba(15, 23, 42, .18);
            }
            .material-dropdown.open {display:block;}
            .material-option {
                display:flex; justify-content:space-between; gap:12px; align-items:flex-start;
                padding:12px 14px; border-bottom:1px solid #edf0f4; cursor:pointer;
            }
            .material-option:last-child {border-bottom:0;}
            .material-option:hover, .material-option.active {background:#eef6ff;}
            .material-option.any {font-weight:bold; background:#fff8e1;}
            .material-option.any:hover, .material-option.any.active {background:#ffefb8;}
            .material-option-badge {
                flex:0 0 auto; padding:3px 7px; border-radius:999px; background:#e8f1ff;
                color:#27588f; font-size:11px; font-weight:bold;
            }
            .material-option.any .material-option-badge {background:#111; color:#fff;}
            .material-empty {padding:16px; color:#667085;}
            @media (max-width: 700px) {
                ...
                #map {height:360px;}
                .material-dropdown {max-height:300px;}
            }
        </style>
    </head>
    <body>
        <div class="container">
            ...
            <form method="POST" class="card" id="materials-calculator-form">
                ...
                <div class="material-search">
                    <input
                        ...
                        type="search"
                        value=\"""" + escape(selected_material_label, quote=True) + """\"
                        placeholder="Начните писать: керамзит, песок, щебень..."
                        autocomplete="off"
                        aria-autocomplete="list"
                        aria-controls="material-dropdown"
                        required
                    >
                    <input
                        id="material-search-value"
                        type="hidden"
                        name="sand_type"
                        value=\"""" + escape(selected_material_value, quote=True) + """\"
                    >
                    <div id="material-dropdown" class="material-dropdown" role="listbox"></div>
                    <small class="material-hint">
                        Сначала показывается «любой» материал выбранного вида, затем все точные марки и фракции.
                    </small>
                </div>
                <noscript>
        ...
        <script>
            const materialSuggestions = """ + material_suggestions_json + """;
            const materialInput = document.getElementById("material-search-input");
            const materialValue = document.getElementById("material-search-value");
            const materialDropdown = document.getElementById("material-dropdown");
            const materialsForm = document.getElementById("materials-calculator-form");
            let visibleMaterialOptions = [];
            ...
            function normalizeMaterialSearch(value) {
                return String(value || "")
                    .toLowerCase()
                    .replaceAll("ё", "е")
                    .replaceAll("кермазит", "керамзит")
                    .replaceAll("керамзид", "керамзит")
                    .replaceAll("щебенка", "щебень")
                    .replace(/[^a-zа-я0-9]+/g, " ")
                    .trim()
                    .replace(/\\s+/g, " ");
            }

            function materialMatches(searchText, tokens) {
                return tokens.every(function(token) {
                    return searchText.includes(token);
                });
            }

            function findMaterialOptions(query) {
                const normalized = normalizeMaterialSearch(query);
                if (!normalized) {
                    return materialSuggestions.filter(function(item) {
                ...
                materialSuggestions.forEach(function(item) {
                    if (item.kind === "any" && item.family !== "all" && materialMatches(item.search, tokens)) {
                        matchedFamilies.add(item.family);
                    }
                });
            ...
            function chooseMaterial(item) {
                materialInput.value = item.label;
                materialValue.value = item.value;
                materialDropdown.classList.remove("open");
                materialInput.setAttribute("aria-expanded", "false");
            }

            function renderMaterialOptions() {
                visibleMaterialOptions = findMaterialOptions(materialInput.value);
                activeMaterialIndex = 0;
                materialDropdown.replaceChildren();

                if (!visibleMaterialOptions.length) {
                    const empty = document.createElement("div");
                    empty.className = "material-empty";
                    empty.textContent = "Совпадений нет. Попробуйте написать только название материала.";
                    materialDropdown.appendChild(empty);
                    materialValue.value = "";
                } else {
                    // Если пользователь просто печатает, первым автоматически выбирается «любой».
                    materialValue.value = visibleMaterialOptions[0].value;
                    visibleMaterialOptions.forEach(function(item, index) {
                        const option = document.createElement("div");
                        option.className = "material-option " + (item.kind === "any" ? "any " : "") + (index === 0 ? "active" : "");
                        ...
                        badge.className = "material-option-badge";
                        badge.textContent = item.kind === "any" ? "Сначала" : "Точная позиция";
            ...
            function updateActiveMaterialOption() {
                const optionNodes = materialDropdown.querySelectorAll(".material-option");
                optionNodes.forEach(function(node, index) {
                    const isActive = index === activeMaterialIndex;
                    node.classList.toggle("active", isActive);
                    node.setAttribute("aria-selected", isActive ? "true" : "false");
                });
                if (optionNodes[activeMaterialIndex]) {
                    optionNodes[activeMaterialIndex].scrollIntoView({block: "nearest"});
                    materialValue.value = visibleMaterialOptions[activeMaterialIndex].value;
                }
            }

            materialInput.addEventListener("focus", renderMaterialOptions);
            materialInput.addEventListener("input", renderMaterialOptions);
            materialInput.addEventListener("keydown", function(event) {
                if (!materialDropdown.classList.contains("open")) {
                    return;
                }
                if (event.key === "ArrowDown" && visibleMaterialOptions.length) {
                    event.preventDefault();
                    activeMaterialIndex = Math.min(activeMaterialIndex + 1, visibleMaterialOptions.length - 1);
                    updateActiveMaterialOption();
                } else if (event.key === "ArrowUp" && visibleMaterialOptions.length) {
                    event.preventDefault();
                    activeMaterialIndex = Math.max(activeMaterialIndex - 1, 0);
                    updateActiveMaterialOption();
                } else if (event.key === "Enter" && visibleMaterialOptions.length) {
                    event.preventDefault();
                    chooseMaterial(visibleMaterialOptions[activeMaterialIndex]);
            ...
            materialsForm.addEventListener("submit", function(event) {
                if (!materialValue.value) {
                    event.preventDefault();
                    materialInput.setCustomValidity("Выберите материал из подсказки.");
